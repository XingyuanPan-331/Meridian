...
async function buildChecklist(userId: string, taskId: string, title: string, description: string | null, taskDepartureAt: Date | null = null) {
  const children = await prisma.task.findMany({
    where: { userId, parentId: taskId },
    select: { id: true, title: true, completedAt: true, departureAt: true },
    orderBy: { sortOrder: "asc" },
  });
  if (children.length > 0) {
    // 2026-08-09（计时模型 V2）：两维度分离——
    // ① minutes = 实际投入（该项 TimeLog.durationSeconds 聚合，用户填/补记的值）
    // ② spanMinutes = 分配时间段（该项完成时刻 − 该项开始时刻；完成早于开始 → 0；开始缺失 → null）
    const logs = await prisma.timeLog.findMany({
      where: { taskId: { in: children.map(c => c.id) } },
      select: { taskId: true, durationSeconds: true },
    });
    const minutesByTask = new Map<string, number>();
    for (const l of logs) minutesByTask.set(l.taskId, (minutesByTask.get(l.taskId) ?? 0) + l.durationSeconds);
    return children.map(c => {
      const start = c.departureAt ?? taskDepartureAt;
      const spent = minutesByTask.get(c.id);
      return {
        id: c.id, text: c.title, done: !!c.completedAt, group: null, noteStep: false, self: false,
        completedAt: c.completedAt?.toISOString() || null,
        // 该项开始时间（子项自己的 departureAt；未开始为 null）
        startedAt: c.departureAt?.toISOString() || null,
        // 实际投入（TimeLog 聚合；无记录 → null）
        minutes: spent ? Math.max(0, Math.round(spent / 60)) : null,
        // 分配时间段（完成 − 开始）
        spanMinutes: c.completedAt && start
          ? Math.max(0, Math.round((c.completedAt.getTime() - start.getTime()) / 60000))
          : null,
      };
    });
  }
...
export async function GET() {
  ...
  const [inProgress, decision, currentState, todayCompleted, todayTimeLogs, todaySchedules, todayTimeline] = await Promise.all([
    ...
    // 2026-08-09 专注时间归属修正：今天 TimeLogs（startedAt 今天）+ 今天完成的任务（completedAt 今天）
    // ——补记场景（出发昨天/今天完成）TimeLog.startedAt 非今天但属于今天完成 → 计入今天专注
    prisma.timeLog.findMany({
      where: { userId, OR: [
        { startedAt: { gte: today, lt: tomorrow } },
        { task: { completedAt: { gte: today, lt: tomorrow } } },
      ] },
    }),
  ...
  if (inProgress) {
    ...
    // 2026-08-12 多段执行：已用 = 全部执行段累计（原只算今天 TimeLog——分段任务只显示今天段，
    // 非总时长；"总完成时间 = 各段之和"）
    const tLogsAll = await prisma.timeLog.findMany({ where: { taskId: inProgress.id, userId: session.user.id }, select: { durationSeconds: true } });
    const elapsedSeconds = tLogsAll.reduce((sum, l) => sum + l.durationSeconds, 0);
  ...
  const enhanceCard = async (m: { taskId: string; title: string }) => {
    ...
    const [task, rawChildren, purpose] = await Promise.all([
      ...
      prisma.task.findMany({
        ...
        select: { id: true, title: true, completedAt: true, departureAt: true },
        orderBy: { sortOrder: "asc" },
      }).then((list) => list.map((c) => ({ id: c.id, text: c.title, done: !!c.completedAt, group: null, noteStep: false, self: false, completedAt: c.completedAt?.toISOString() || null, startedAt: c.departureAt?.toISOString() || null }))),
      resolvePurposeFinal(userId, { purpose: null, parentId: m.taskId }).catch(() => null),
    ]);
    if (!task) return m;
    // 2026-08-09（计时模型 V2）：两维度分离——minutes = 实际投入（TimeLog 聚合）；
    // spanMinutes = 分配段（完成 − 该项开始，兜底任务出发）
    const dep = task.departureAt ? new Date(task.departureAt).getTime() : null;
    const childIds = rawChildren.map((c) => c.id);
    const spentLogs = await prisma.timeLog.findMany({
      where: { taskId: { in: childIds } },
      select: { taskId: true, durationSeconds: true },
    });
    const minutesByTask = new Map<string, number>();
    for (const l of spentLogs) minutesByTask.set(l.taskId, (minutesByTask.get(l.taskId) ?? 0) + l.durationSeconds);
    const children = rawChildren.map((c) => {
      const startMs = c.startedAt ? new Date(c.startedAt).getTime() : dep;
      const spent = minutesByTask.get(c.id);
      return {
        ...c,
        // 实际投入（TimeLog 聚合；无记录 → null）
        minutes: spent ? Math.max(0, Math.round(spent / 60)) : null,
        // 分配段（完成 − 开始；开始缺失 → null）
        spanMinutes: c.completedAt && startMs !== null
          ? Math.max(0, Math.round((new Date(c.completedAt).getTime() - (startMs as number)) / 60000))
          : null,
      };
    });
    const sched = todaySchedules.find((s) => s.taskId === m.taskId);
    // 2026-08-09 左栏已用时间修正：mustDo 兜底卡也聚合 TimeLog（此前恒 0 → 显示"—"）
    const selfLogs = await prisma.timeLog.aggregate({
      where: { taskId: m.taskId },
      _sum: { durationSeconds: true },
    });
    // 2026-08-10 归属修复：mustDo 卡也派生 parentTitle（此前 null → 主卡"无归属"）
    const parentTitles: string[] = [];
    let pid = task.parentId;
    let guard = 0;
    while (pid && guard < 6) {
      const p = await prisma.task.findUnique({ where: { id: pid }, select: { title: true, parentId: true } });
      if (!p) break;
      parentTitles.unshift(p.title);
      pid = p.parentId; guard++;
    }
    return {
      ...m,
      children,
      ...
      parentTitle: parentTitles.length ? parentTitles.join(" / ") : null,
      scheduledStart: sched?.scheduledStart?.toISOString() || null,
      scheduledEnd: sched?.scheduledEnd?.toISOString() || null,
      elapsedMinutes: Math.round((selfLogs._sum.durationSeconds ?? 0) / 60),
    };
  };
