...
/* ── 主组件 ── */
export function FocusCardV2({ card, onStart, onComplete, onCompleteItem, onItemStart, onItemToggle, onItemAdd, onItemMove, onItemReorder, onCheckin, onSkip, onPause, onContinueTomorrow, busy, demo, advice }: Props) {
      ...
      <div className="bg-white rounded-[14px] border border-[var(--v2-border)] overflow-hidden shadow-[0_1px_3px_rgba(16,24,40,0.05)]" style={doneFlash ? { boxShadow: "0 0 0 2px #16a34a" } : undefined}>
        ...
        <div className="fcv2-grid">
          ...
          <div className="fcv2-left px-4 py-3 space-y-2.5 min-w-0">
            ...
            {/* 2026-08-09：AI 执行建议（左栏 · 设计稿预留位；后端 executionAdvice 接线） */}
            {advice?.message && (
            ...
            {isList && card.stages && card.stages.length > 0 && (
              <div>
                ...
                <ul className="space-y-px">
                  {card.stages.map((s) => (
                    <li key={s.name} className={`flex items-center gap-1.5 text-[12px] px-1.5 py-1 rounded-md ${
