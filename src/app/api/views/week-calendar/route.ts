...
export async function GET(req: NextRequest) {
  ...
  const [timeLogs, scheduleEntries, allActiveTasks, userModel] = await Promise.all([
    ...
    prisma.schedule.findMany({ where: { userId, scheduledStart: { gte: weekStart, lt: weekEnd } }, include: { task: { select: { id: true, title: true, taskType: true, status: true, importance: true, category: true, tags: true, theme: true, themeColor: true, estimatedMinutes: true, source: true, deadline: true, description: true, level: true, accumulate: true, completedAt: true, departureAt: true, actualMinutes: true } } }, orderBy: { scheduledStart: "asc" } }),
    prisma.task.findMany({ where: { userId, status: { in: ["not_started", "in_progress", "delayed"] } }, orderBy: [{ importance: "desc" }, { deadline: "asc" }], include: { children: { select: { id: true, title: true, status: true, completedAt: true } } } }),
  ...
  // 2026-08-12 执行段按任务聚合（暂停分段/多段执行：plan 对多段任务显示多块）
  const segMap = new Map<string, { id: string; startedAt: string; endedAt: string; durationSeconds: number }[]>();
  for (const log of timeLogs) {
    if (log.type === "segment" && log.endedAt && log.durationSeconds > 0) {
      const arr = segMap.get(log.taskId) ?? [];
      arr.push({ id: log.id, startedAt: log.startedAt.toISOString(), endedAt: log.endedAt!.toISOString(), durationSeconds: log.durationSeconds });
      segMap.set(log.taskId, arr);
    }
  }
